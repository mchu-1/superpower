# Superpower early team — non-biomed diligence drafts

**Company:** Superpower (superpower.com) — preventative health membership / “health super-app” (labs + AI + concierge). Founded 2023; pre-seed May 2024; Series A / public launch Apr 2025.  
**Lane:** Core/founding team members **without biomedical training** (product, design, eng, growth).  
**Seed subject:** Hannah Ahn (Founding Designer / Head of Design).  
**Output date:** 2026-09-09 (AEST / UTC+10).  
**GitHub:** Tracked under `sources/special-topics/early-team/` on main.

---

## Files

| Path | Contents |
|---|---|
| [`hannah-ahn.md`](./hannah-ahn.md) | Hannah Ahn public profile: LinkedIn, X, role, prior companies, education, non-biomed assessment, labeled claims |
| [`hannah-ahn-inbound.md`](./hannah-ahn-inbound.md) | On-site inbound pack: Head of Design byline, `/hannah` → `via=super-hannah` founder-class vanity, adjacent design cluster |
| [`early-non-biomed-roster.md`](./early-non-biomed-roster.md) | Table of early non-biomed candidates + confidence; explicit biomed exclusions |
| [`README.md`](./README.md) | This index |

Absolute paths on the box:

- `/workspace/superpower-early-team/hannah-ahn.md`
- `/workspace/superpower-early-team/early-non-biomed-roster.md`
- `/workspace/superpower-early-team/README.md`

---

## Method

- WebSearch + WebFetch; curl for X meta / pages that block fetch providers.  
- Prefer primary: LinkedIn posts/profiles (via public snippets), company press (TechCrunch, BusinessWire, Superpower changelog), personal X bios.  
- Aggregators (Village, The Org, Happenstance, Humantic) marked **[secondary]**.  
- Every material claim labeled; **[inferred]**, **[gap]**, **[off-site]** called out.  
- No invention of titles, dates, or degrees.

---

## Key findings (executive)

- **Hannah Ahn** is publicly **Founding Designer (from ~Jul 2024)** and **Head of Design (from ~Apr 2025)** at Superpower; X **@hannah_ahn**; LinkedIn **linkedin.com/in/ahnhannah**. Non-biomed: Canva PM → design agency (Flair) → Superpower; UNSW CS/Science discontinued. Email publicly listed as hannah@superpower.com on Read.cv indexes.  
- **Three co-founders** (Jacob Peters, Max Marchione, Kevin Unkrich) are repeatedly named in TechCrunch/BusinessWire/Forbes; education/career paths are **commerce / CS-engineering**, not medical school. X: **@J__Cub**, **@maxmarchione**, **@kevinunkrich**.  
- **Nikita Mashchenko** claims **first hire / founding engineer** from **Dec 2023** (CS, UIUC) — strongest non-founder eng early signal.  
- Other strong non-biomed early candidates: **Amada Reyes** (Founding Team Jan 2024, GTM/community), **Albe Tremblay** (Founding GTM Partnerships Aug 2024).  
- **Exclude from this lane:** Manoj Arachige MD (Head of Product), Anant Vinjamoori MD (Chief Longevity Officer), Jonathan Richina MD (early physician quote).  
- Press launch pieces name founders, not design/eng bench — roster beyond founders rests heavily on LinkedIn/self-primary evidence.

---

## Limitations

- LinkedIn full pages are partially gated; Village/search snippets used where noted.  
- Read.cv returned 402; hannah-ahn.com returned deployment disabled at fetch time.  
- Team size and titles move quickly (~80–90 employees per LinkedIn company snippets); “founding” titles continued into 2025.
